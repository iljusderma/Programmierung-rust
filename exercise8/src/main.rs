...
fn read_old_library_file() -> Result<LibraryOld, Box<dyn Error>> {
...
fn read_new_library_file() -> Result<Library, Box<dyn Error>> {
    let library_file = File::open("library.json")?;
    let reader = BufReader::new(library_file);
...
fn main() {
    ...
    let library_old = match read_old_library_file() {
        Ok(library) => library,
        Err(e) => panic!("{:?}", e),
    };
    ...
    /*
    let mut library = match read_new_library_file() {
        Ok(library) => library,
        Err(e) => panic!("{:?}", e),
    };
    */
