...
pub fn convert(library_old: LibraryOld) -> Library{
    ...
    for item in library_old.items{
        match item{
            ...
            ItemOld::Newspaper(old_newspaper) => {
                library_new.add(Item::Newspaper(Newspaper::from(old_newspaper)))
            },
            ItemOld::Movie(old_movie) => {
                let directors_uuid = *library_new.persons_uuid(&old_movie.director).unwrap();
                //library_new.add(Item::Movie(Movie::new(title, year, directors_uuid)));
                let mut new_movie = Movie::from(old_movie);
                new_movie.director = directors_uuid;
                library_new.add(Item::Movie(new_movie))
            },
        };
    }
    library_new
